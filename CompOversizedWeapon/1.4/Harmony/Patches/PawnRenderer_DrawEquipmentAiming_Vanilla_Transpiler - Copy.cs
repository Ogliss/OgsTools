...
namespace OgsCompOversizedWeapon
{
    ...
    public static class PawnRenderer_DrawEquipmentAiming_Transpiler
	{
		...
		public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				{
					if (Prefs.DevMode) Log.Message($"Oversized: Transpiler {i} Graphics.DrawMesh: {list[i - 5].opcode}({((LocalBuilder)list[i - 5].operand)?.GetType()}), {list[i - 4].opcode}({list[i - 4].operand}), {list[i - 3].opcode}({list[i - 3].operand}), {list[i - 2].opcode}({list[i - 2].operand}), {list[i - 1].opcode}({list[i - 1].operand})");
				}
			}
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				if (instruction.OperandIs(AccessTools.Method(type: typeof(Graphics), name: nameof(Graphics.DrawMesh), parameters: new[] { typeof(Mesh), typeof(Vector3), typeof(Quaternion), typeof(Material), typeof(Int32) })))
				{
					...
					instruction = new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(PawnRenderer_DrawEquipmentAiming_Transpiler), "DrawMeshModified", null, null));
		...
		public static void DrawMeshModified(Mesh mesh, Vector3 position, Quaternion rotation, Material mat, int layer, Thing eq, float aimAngle)
		{
			...
			if (DualWeapon)
			{
				...
				PawnRenderer_DrawEquipmentAiming_Transpiler.DrawEquipmentAimingOverride(mesh, eq, drawLoc, offHandAngle, compOversized, equippable, pawn);
			...
			if (Aiming && localTargetInfo != null)
			{
				...
				PawnRenderer_DrawEquipmentAiming_Transpiler.DrawEquipmentAimingOverride(mesh, thingWithComps, drawLoc2, (DualWeapon ? mainHandAngle : aimAngle), compOversized, equippable, pawn, DualWeapon);
			}
			else
			{
				...
				PawnRenderer_DrawEquipmentAiming_Transpiler.DrawEquipmentAimingOverride(mesh, thingWithComps,  drawLoc2, (DualWeapon ? mainHandAngle : aimAngle), compOversized, equippable, pawn, false);
			}
