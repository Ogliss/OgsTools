...
namespace OgsCompOversizedWeapon
{
    ...
    public static class OversizedUtil
    {
		...
		public static void DrawSizeModified(Thing eq, ref Mesh mesh)
		{
			mesh = mesh == MeshPool.plane10 ? MeshPool.GridPlane(eq.Graphic.drawSize) : MeshPool.GridPlaneFlip(eq.Graphic.drawSize);
		}
        ...
        {/*
            ...
            if (eq.def.apparel != null)
            {
				Vector2 offset = eq.def.apparel.wornGraphicData.BeltOffsetAt(pawn.Rotation, pawn.story.bodyType);
			...
			*/
            ...
            {
                if (eq.def.graphicData.drawSize != Vector2.zero)
				{
				//	Log.Message(pawn.Name + " oversized " + eq.def.LabelCap + " def drawSize: "+ eq.def.graphicData.drawSize+ " Graphic drawSize: " + eq.Graphic.drawSize);
					Graphics.DrawMesh(mesh, Matrix4x4.TRS(position, rotation, new Vector3(eq.def.graphicData.drawSize.x, 0f, eq.def.graphicData.drawSize.y)), mat, layer);
				}
				else
