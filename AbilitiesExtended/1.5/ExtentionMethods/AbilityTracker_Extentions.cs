	...
	public static class AbilityTracker_Extentions
	{
        ...
        public static bool isPsyker(Pawn pawn, out int Level, out float Mult)
        {
            ...
            if (pawn.RaceProps.Humanlike)
            {
                ...
                if (pawn.story.traits.HasTrait(AbilitesExtendedTraitDefOf.PsychicSensitivity))
                {
                    result = pawn.story.traits.DegreeOfTrait(AbilitesExtendedTraitDefOf.PsychicSensitivity) > 0;
                    Level = pawn.story.traits.DegreeOfTrait(AbilitesExtendedTraitDefOf.PsychicSensitivity);
                }
                else
                {
