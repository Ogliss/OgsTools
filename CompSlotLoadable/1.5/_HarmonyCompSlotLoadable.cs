...
namespace OgsCompSlotLoadable
{
    ...
    internal static class HarmonyCompSlotLoadable
    {
        ...
        {
            var harmony = new Harmony("ogstools.Ogliss.comps.slotloadable");
            ...
            harmony.Patch(AccessTools.Method(typeof(StatExtension), nameof(StatExtension.GetStatValue), new Type[] { typeof(Thing), typeof(StatDef), typeof(bool), typeof(int) }), null,
        ...
        public static void ApplyHealing(Thing thing, int woundLimit = 0, Thing vampiricTarget = null)
        {
            if (thing is Pawn pawn)
            {
                ...
                foreach (var rec in pawn.health.hediffSet.GetInjuredParts())
                    ...
                    {
                        pawn.health.hediffSet.GetHediffs<Hediff_Injury>(ref injuries, x => x.Part == rec);
                        foreach (var current in injuries)
                        {
