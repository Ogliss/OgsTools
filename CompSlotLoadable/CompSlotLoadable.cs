...
namespace OgsCompSlotLoadable
{
    public class CompSlotLoadable : ThingComp
    {
        ...
        private void TryCancel(string reason = "")
        {
            ...
            if (pawn != null)
            {
                if (pawn.CurJob.def == CompSlotLoadableDefOf.AdeptusMechanicusGatherSlotItem)
                    pawn.jobs.StopAll();
                isGathering = false;
        ...
        private void TryGiveLoadSlotJob(Thing itemToLoad)
        {
            if (GetPawn != null)
                if (!GetPawn.Drafted)
                {
                    ...
                    var job = new Job(CompSlotLoadableDefOf.AdeptusMechanicusGatherSlotItem, itemToLoad)
                    {
                        count = 1
                    };
