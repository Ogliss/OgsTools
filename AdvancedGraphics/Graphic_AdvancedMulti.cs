...
namespace AdvancedGraphics
{
	...
	public class Graphic_AdvancedMulti : Graphic
	{
		...
		public override void Init(GraphicRequest req)
		{
			...
			string reqpath = req.graphicData.texPath;
			String[] elements = Regex.Split(reqpath, "/");
			List<Texture2D> list = (from x in ContentFinder<Texture2D>.GetAllInFolder(reqpath)
									where !x.name.EndsWith(Graphic_AdvancedMulti.MaskSuffix) && !x.name.EndsWith(Graphic_AdvancedMulti.GlowSuffix) && !x.name.EndsWith(Graphic_AdvancedMulti.GlowMaskSuffix) && !x.name.Contains(Graphic_AdvancedMulti.NSuffix) && !x.name.Contains(Graphic_AdvancedMulti.SSuffix) && !x.name.Contains(Graphic_AdvancedMulti.ESuffix) && !x.name.Contains(Graphic_AdvancedMulti.WSuffix)
									orderby x.name
									select x).ToList<Texture2D>();
			if (list.NullOrEmpty<Texture2D>())
			{
				Log.Error("Collection cannot init: No textures found at path " + reqpath, false);
		...
		{
			return GraphicDatabase.Get<Graphic_AdvancedMulti>(this.path, newShader, this.drawSize, newColor, newColorTwo, this.data);
		}
