...
namespace CompTurret
{
	...
	public class Verb_ShootCompMounted : Verb_LaunchProjectile
	{
		...
		public override bool TryCastShot()
		{
			...
			if (f)
			{
				shootLine.ChangeDestToMissWild_NewTemp(shotReport.AimOnTargetChance_StandardTarget, projectile.projectile.flyOverhead, this.Caster.Map);
		...
		private new bool CanHitFromCellIgnoringRange(IntVec3 sourceCell, LocalTargetInfo targ, out IntVec3 goodDest)
		{
			if (targ.Thing != null)
			{
				...
				ShootLeanUtility.CalcShootableCellsOf(Verb_ShootCompMounted.tempDestList, targ.Thing, this.Caster.Position);
