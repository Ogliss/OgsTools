...
//	[StaticConstructorOnStartup]
	public class TunnelHiveSpawner : ThingWithComps
	{
		...
		public TunnelExtension Ext => ext ??= this.def.HasModExtension<TunnelExtension>() ? this.def.GetModExtension<TunnelExtension>() : null;
		...
		public override void Tick()
		{
			...
			Rand.PushState();
			if (Rand.MTBEventOccurs(FilthSpawnMTB, 1f, 1.TicksToSeconds()) && CellFinder.TryFindRandomReachableCellNearPosition(base.Position, base.Position, base.Map, FilthSpawnRadius, TraverseParms.For(TraverseMode.NoPassClosedDoors), null, null, out IntVec3 result) && !filthTypes.NullOrEmpty())
			{
			...
			if (spawnHive)
			{
				...
				if (obj != null)
				{
					...
					CompSpawnerPawn spawnerPawn = obj.TryGetComp<CompSpawnerPawn>();
					if (spawnerPawn != null)
					{
						spawnerPawn.initialPawnsPoints = spawnerPawn.Props.initialPawnsPoints;
					//	if (Prefs.DevMode) Log.Message($"{obj.LabelCap} SpawnPawn initialPawnsPoints: {spawnerPawn.initialPawnsPoints}");
						spawnerPawn.SpawnInitialPawns();
					}
				}
			}
			...
			//	if (Prefs.DevMode) Log.Message($"{this.LabelCap} TunnelHiveSpawner initialPoints: {initialPoints}");
		...
		public override void ExposeData()
		{
			...
			Scribe_Values.Look(ref initialPoints, "initialPoints", 0f);
