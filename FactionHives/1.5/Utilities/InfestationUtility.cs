...
namespace ExtraHives
{
	public static class InfestationUtility
    {
		public static Thing SpawnTunnels(ThingDef hiveDef, int hiveCount, Map map, bool spawnAnywhereIfNoGoodCell = false, bool ignoreRoofedRequirement = false, string questTag = null, Faction faction = null)
		{
			...
			InfestationCellFinder.InfestationParms infestationParms = new InfestationCellFinder.InfestationParms(HiveExt);
			if (!InfestationCellFinder.TryFindCell(out IntVec3 cell, map, infestationParms))
			{
			...
			if (HiveExt?.TunnelDef!=null)
			{
				TunnelDef = HiveExt.TunnelDef;
			...
			CompSpawnerPawn spawnerPawn = thing.TryGetComp<CompSpawnerPawn>();
            if (spawnerPawn != null)
            {
				if (Prefs.DevMode) Log.Message($"{thing.LabelCap} spawnerPawn initialPawnsPoints: {spawnerPawn.initialPawnsPoints}");
			}
